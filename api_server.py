"""
api_server.py — API Flask SoH + proxy MongoDB
"""
import os
import pickle
import numpy as np
...
def index():
    return jsonify({
        ...
        "endpoints": {
            ...
            "POST /predict"      : "SoH prediction (single sequence)",
            "POST /predict/batch": "SoH prediction (batch)",
            "GET  /mongo/data"   : "MongoDB proxy (needs MONGO_URI env var)",
        }
    })
...
def mongo_data():
    ...
    if not mongo_uri:
        return jsonify({
            "error": "Variable MONGO_URI non configurée. "
                     "Ajoutez-la dans Render → Settings → Environment Variables."
        }), 503
...
# ─── Point d'entrée (gunicorn lit directement 'app') ──────────
# Gunicorn est lancé via Procfile : gunicorn api_server:app --bind 0.0.0.0:$PORT
# Le bloc __main__ sert uniquement pour les tests locaux
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 5000))
    print(f"Démarrage local sur port {port}")
